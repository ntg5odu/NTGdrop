...
// Import variables
const { seed, currency, issuer, sendingMemo } = require('../config.json');
const fs = require('fs');
...
const send = async (userList) => {
  // Log the userList for debugging
  ...
  try {
    ...
    for (let i = 0; i < userList.length; i++) {
      ...
      const counter = i + 1;
      console.log(`Processing user ${counter}:`, JSON.stringify(user, null, 2));
