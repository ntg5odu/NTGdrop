...
// Import variables
const { seed, currency, currencyIssuer, sendingMemo } = require('../configNFTrait.json');
const fs = require('fs');
    ...
    try {
        ...
        const prepared = await client.autofill({
            ...
            Amount: {
                ...
                value: user.totalAmount.toString(), // Airdrop amount per user
            },
            Destination: user.holderAddress,
        ...
        console.error(`Error in send function for user ${user.holderAddress}:`, error.message);

        // Log error details to JSON file
        logTransaction({
            counter,
            account: user.holderAddress,
...
const fetchTransactionResult = async (client, tx, retries = 10) => {
    try {
        ...
        if (result.result && result.result.meta) {
            ...
            logTransaction({
                counter: tx.counter,
                account: tx.user.holderAddress,
                amount: tx.user.totalAmount,
...
const send = async () => {
    ...
    // Filter the user list to include only qualified users with trustline and not ignored
    const userList = snapshot.holders.filter(holder => holder.isQualified && holder.hasTrustline && !holder.ignored);
    ...
    try {
        ...
        const totalAirdropAmount = userList.reduce((sum, user) => sum + user.totalAmount, 0);
